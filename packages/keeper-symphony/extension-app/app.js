...
import {THEME_TYPES, Logger, RestClient} from "symphony-bdk-ui-toolkit";
...
const appWrapper = async () => {
  ...
  SYMPHONY.remote
        ...
        .then((response) => {
          ...
          ReactDOM.render(
            <Provider store={store}>
              <Routes
                userId={userId}
                jwtService={extendedUserInfoService || MOCK_USER_SERVICE}
              />
            </Provider>,
    ...
    .catch((error) => {
      throw new Error(
        "Unable to reach the data for Extension App, please verify the Authentication with Server",
        error
      );
    });
};
