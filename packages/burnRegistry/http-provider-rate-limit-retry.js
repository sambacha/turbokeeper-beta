...
function log() {
  if (DEBUG) {
    let logArgs = [new Date().toISOString()].concat(
 ...
 */
var HTTPProviderRateLimitRetry = function HTTPProviderRateLimitRetry(
  host,
  options
) {
  options = options || {};
 ...
 */
HTTPProviderRateLimitRetry.prototype.attemptSend = function (
  payload,
  callback
) {
  var _this = this;
