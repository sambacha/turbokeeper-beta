      ...
      prompt.ask(async (answer) => {
        if (answer) {
          ...
          await crvStrategyKeep3r.addStrategy(
            config.contracts.mainnet.comp.address,
          ...
          console.log(
            `calculating harvest for: ${Object.keys(
              strategies
            )}. please wait ...`
          );
          ...
          console.log(
            "checking if workable for: ycrv, busd, sbtc, 3pool. please wait ..."
          );
          console.time("workable");
          for (const strategy in strategies) {
            console.log(
              `workable(${strategy})`,
              await crvStrategyKeep3r.callStatic.workable(
                strategies[strategy].contract.address
              )
            );
          }
