...
describe("CrvStrategyKeep3r", function () {
  ...
  it.only("Should deploy on mainnet fork", async function () {
    ...
    const crvStrategyKeep3r = (
      await CrvStrategyKeep3r.deploy(config.contracts.mainnet.keep3r.address)
    ).connect(owner);

    // Setup crv strategies
    const ycrvContract = await ethers.getContractAt(
      "StrategyCurveYVoterProxy",
      config.contracts.mainnet.ycrv.address,
      owner
    );
    const busdContract = await ethers.getContractAt(
      "StrategyCurveYVoterProxy",
      config.contracts.mainnet.busd.address,
      owner
    );
    const sbtcContract = await ethers.getContractAt(
      "StrategyCurveYVoterProxy",
      config.contracts.mainnet.sbtc.address,
      owner
    );
    const pool3Contract = await ethers.getContractAt(
      "StrategyCurveYVoterProxy",
      config.contracts.mainnet.pool3.address,
      owner
    );
    ...
    await crvStrategyKeep3r.addStrategy(
      pool3Contract.address,
      requiredHarvestAmount
    );
    console.timeEnd("addStrategy");

    console.time("updateRequiredHarvestAmount");
    await crvStrategyKeep3r.updateRequiredHarvestAmount(
      ycrvContract.address,
      requiredHarvestAmount.mul(2)
    );
    await expect(
      crvStrategyKeep3r.updateRequiredHarvestAmount(ycrvContract.address, 0)
    ).to.be.revertedWith(
    ...
    await expect(
      crvStrategyKeep3r.removeStrategy(ycrvContract.address)
    ).to.be.revertedWith(
      "crv-strategy-keep3r::remove-strategy:strategy-not-added"
    );
    await expect(
      crvStrategyKeep3r.updateRequiredHarvestAmount(
        ycrvContract.address,
        requiredHarvestAmount
      )
    ).to.be.revertedWith(
    ...
    await expect(
      crvStrategyKeep3r.callStatic.workable(ycrvContract.address)
    ).to.be.revertedWith("crv-strategy-keep3r::workable:strategy-not-added");
    console.timeEnd("removeStrategy");

    console.time("addStrategy");
    await expect(
      crvStrategyKeep3r.addStrategy(ycrvContract.address, 0)
    ).to.be.revertedWith(
      "crv-strategy-keep3r::set-required-harvest:should-not-be-zero"
    );
    await crvStrategyKeep3r.addStrategy(
    ...
    await expect(
      crvStrategyKeep3r.addStrategy(ycrvContract.address, requiredHarvestAmount)
    ).to.be.revertedWith(
    ...
    console.log(
      "workable(ycrv)",
      await crvStrategyKeep3r.callStatic.workable(ycrvContract.address)
    );
    console.log(
      "workable(busd)",
      await crvStrategyKeep3r.callStatic.workable(busdContract.address)
    );
    console.log(
      "workable(sbtc)",
      await crvStrategyKeep3r.callStatic.workable(sbtcContract.address)
    );
    console.log(
      "workable(pool3)",
      await crvStrategyKeep3r.callStatic.workable(pool3Contract.address)
    );
    console.timeEnd("workable");

    console.time("harvest should revert on ycrv");
    await expect(
      crvStrategyKeep3r.harvest(ycrvContract.address)
    ...
    await ycrvContract
      .connect(multisig)
      .setStrategist(crvStrategyKeep3r.address);
    await busdContract
      .connect(multisig)
      .setStrategist(crvStrategyKeep3r.address);
    await sbtcContract
    ...
    expect(await crvStrategyKeep3r.callStatic.workable(ycrvContract.address)).to
      .be.false;
    console.timeEnd("forceHarvest ycrv makes workable false");

    console.time("keeper harvest reverts with not-workable");
    await expect(
      crvStrategyKeep3r.connect(keeper).harvest(ycrvContract.address)
